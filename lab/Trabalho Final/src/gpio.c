...
volatile bool flag_gpio1A, flag_gpio1B, flag_gpio2A, flag_gpio2B, flag_gpio3A;
/*-----------------------------------------------------------------------------
      		...
      		switch(mod){
        		case GPIO0:
         		...
         		case GPIO3:
            			...
            			while((ckmGetCLKModuleRegister(CKM_PER, CKM_PER_GPIO3_CLKCTRL) & (0x3<<16)) != 0) 
            		break;
         		default:
...
/* 
 * ===  FUNCTION  ======================================================================
 *         Name:  gpioPinMuxSetupPin
 *  Description:  
 * =====================================================================================
 */
void gpioPinMuxSetup(gpioMod mod, ucPinNumber pin){
...
void gpioIsrHandlerA(gpioMod mod, ucPinNumber pin){
    ...
    case GPIO2:
        ...
        break;
	case GPIO3:
		uartPutString(UART0,"TESTE\n\r",7);
		HWREG(SOC_GPIO_3_REGS+GPIO_IRQSTATUS_0) = (1<<pin);
		setFlag_gpio3A(true);
		break;
    default:
        // Código a ser executado caso o irq_number não seja nenhum dos casos anteriores
        break;
	}
}

void gpioIsrHandlerB(gpioMod mod, ucPinNumber pin){
	switch (mod) {
    case GPIO0:
        break;
    case GPIO1:
...
void  settingIntGpioA(gpioMod mod, ucPinNumber pin){
    switch (mod) {
	case GPIO0:
        break;
    case GPIO1:
...
void  settingIntGpioB(gpioMod mod, ucPinNumber pin){
    switch (mod) {
	case GPIO0:
        break;
    case GPIO1:
...
unsigned int readBut(gpioMod mod, ucPinNumber pin) {
	switch (mod) {
	case GPIO0:
		return 0;
		break;
	case GPIO1:
		return (HWREG(SOC_GPIO_1_REGS+GPIO_DATAIN) & 1<<pin);
		break;
	case GPIO2:
		return (HWREG(SOC_GPIO_2_REGS+GPIO_DATAIN) & 1<<pin);
		break;
	case GPIO3:
		return (HWREG(SOC_GPIO_3_REGS+GPIO_DATAIN) & 1<<pin);
		break;
	default:
		return 0;
		break;
...
void setFlag_gpio1A(bool flag) {
    flag_gpio1A = flag;
}

void setFlag_gpio1B(bool flag) {
    flag_gpio1B = flag;
}

void setFlag_gpio2A(bool flag) {
    flag_gpio2A = flag;
}

void setFlag_gpio2B(bool flag) {
    flag_gpio2B = flag;
}

void setFlag_gpio3A(bool flag) {
    flag_gpio3A = flag;
}
